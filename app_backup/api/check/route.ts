...
export async function POST(request: Request) {
    try {
        ...
        const prompt = "この画像から車のナンバープレートの文字を読み取り、「富山500あ1234」のようなスペースのない連続した文字列のみを出力してください。ナンバープレートが写っていない、または読み取れない場合は、必ず「読取不可」という4文字のみを出力してください。それ以外の説明文や謝罪文は一切不要です。";
        ...
        let detectedPlate = result.response.text().trim();
        let finalStatus = "";
        let isOk = false;
